...
instanceof	Instance Comparator	returns true if a reference variable is pointing to an object of that type
Bitwise Operators
...
IS-A relationship: A class that inherits from another class forms an IS-A relationship with the parent class (ex. Dog is an Animal)
...
Containing related state and behavior together in a class
